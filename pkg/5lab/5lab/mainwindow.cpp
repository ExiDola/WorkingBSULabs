...
{
    ui->setupUi(this);
    QString gradient = "background: qradialgradient(cx: 0.5, cy: 0.5, radius: 0.5, "
                       "fx: 0.5, fy: 0.5, stop: 0 rgba(173, 216, 230, 200), "
                       "stop: 1 rgba(135, 206, 250, 150));";
    ...
    g->addWidget(ui->clippingText, 11, 1, 1, 2);
    g->addWidget(ui->segments, 12, 1, 1, 2);
    g->addWidget(ui->poly, 13, 1, 1, 2);
    centralWidget()->setLayout(g);
    setMinimumSize({600, 600});
    setWindowTitle("Алгоритмы отсечения");
}
...
void MainWindow::ProcessSegments()
{
    ...
    for (int i = 0; i < n; ++i)
    {
        ...
        {
            //outside
            area -> AddLineSegment(LineSegmentData{QPointF(x1, y1), QPointF(x2, y2), customColor});
        }
        else
        {
            qreal A = y2 - y1;
            ...
            while(code1 != 0 || code2 != 0)
            {
                ...
                {
                    qreal y = (-C - A * xmin) / B;
                    area -> AddLineSegment(LineSegmentData{QPointF(x1, y1), QPointF(xmin, y), customColor});
                    x1 = xmin;
                    y1 = y;
                }
                else if (code1 & 2)
                {
                    qreal y = (-C - A * xmax) / B;
                    area -> AddLineSegment(LineSegmentData{QPointF(x1, y1), QPointF(xmax, y), customColor});
                    x1 = xmax;
                    y1 = y;
                ...
                {
                    qreal x = (-C - B * ymin) / A;
                    area -> AddLineSegment(LineSegmentData{QPointF(x1, y1), QPointF(x, ymin), customColor});
                    x1 = x;
                    y1 = ymin;
                }
                else
                {
                    qreal x = (-C - B * ymax) / A;
                    area -> AddLineSegment(LineSegmentData{QPointF(x1, y1), QPointF(x, ymax),customColor});
                    x1 = x;
                    y1 = ymax;
                }
                code1 = getCode(x1, y1);
